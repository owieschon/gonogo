...
Everything else is a local file. `events.jsonl` at the root of this checkout is committed and public, and holds fixture-sweep events only. Real subject events — `kind: real` from `judge`, plus `rater` and `outcome` — default to `private/events.jsonl`, which is gitignored; `gonogo` refuses to write those kinds to `events.jsonl` or anywhere else inside this checkout, and names the private destination when it does. Send them elsewhere with `--events <path>` outside the checkout. Each run also writes its verdict and evidence to `<repo>/runs/<timestamp>/`, or to `--out`.
