...
v0.1.1: one judge backend (`claude -p`), seven author-labeled fixtures, **uncalibrated against human review** — zero same-evidence judge/human pairs exist. The 2026-08-31 live gate failed its scope and claim floors and exposed two attempted-gaming false positives, now covered by negative controls but not yet resampled. Live judging sends the spec, diff, transcript, tests and commits to the provider; inspect its data policy before using private material. This is a regression test, not proof the judge is right. MIT.
