...
Want to see a verdict without any of that? `bun run demo [output directory]` needs only Bun and Git — no `claude` CLI, no API credentials — and writes `verdict.json`/`verdict.html` by replaying a committed recording of the merged-but-wrong fixture. It never calls a live judge.
